...
static int wm8951_add_widgets(struct snd_soc_codec *codec)
{
	...
	for(i = 0; i < ARRAY_SIZE(wm8951_dapm_widgets); i++) {
		snd_soc_dapm_new_control(codec, &wm8951_dapm_widgets[i]);
	}
...
static int wm8951_mute(struct snd_soc_codec_dai *dai, int mute)
{
...
static int wm8951_set_dai_sysclk(struct snd_soc_codec_dai *codec_dai,
...
static int wm8951_set_dai_fmt(struct snd_soc_codec_dai *codec_dai,
		unsigned int fmt)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	u16 iface = 0;
...
struct snd_soc_codec_dai wm8951_dai = {
	.name = "WM8951",
	.capture = {
		.stream_name = "Capture",
		.channels_min = 1,
