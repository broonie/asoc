...
static void playpaq_wm8510_shutdown(
    struct snd_pcm_substream *substream)
{
}
...
static int playpaq_wm8510_hw_params(
    struct snd_pcm_substream *substream,
    struct snd_pcm_hw_params *params)
{
    ...
    if (ret < 0) {
        pr_warning("playpaq_wm8510: Failed to set CODEC DAI format (%d)\n", ret);
        return ret;
    }
    ret = cpu_dai->dai_ops.set_fmt(cpu_dai, fmt);
    if (ret < 0) {
        pr_warning("playpaq_wm8510: Failed to set CPU DAI format (%d)\n", ret);
        return ret;
    }
    ...
    ret = cpu_dai->dai_ops.set_clkdiv(cpu_dai, AT32_SSC_CMR_DIV, cd.cmr_div);
    if (ret < 0) {
    ...
    ret = cpu_dai->dai_ops.set_clkdiv(cpu_dai, AT32_SSC_TCMR_PERIOD,
                                      cd.period);
    if (ret < 0) {
        pr_warning("playpaq_wm8510: Failed to set CPU transmit period (%d)\n",
                   ret);
        return ret;
    }
#endif  /* CONFIG_SND_AT32_SOC_PLAYPAQ_SLAVE */
     ...
     */
    pr_debug("playpaq_wm8510: pll_in = %ld, pll_out = %u, bclk = %x, mclk = %x\n",
             clk_get_rate(CODEC_CLK), pll_out, bclk, mclk_div);

#if !defined CONFIG_SND_AT32_SOC_PLAYPAQ_SLAVE
    ret = codec_dai->dai_ops.set_clkdiv(codec_dai, WM8510_BCLKDIV, bclk);
    if (ret < 0) {
        pr_warning("playpaq_wm8510: Failed to set CODEC DAI BCLKDIV (%d)\n", ret);
        return ret;
    }
#endif  /* CONFIG_SND_AT32_SOC_PLAYPAQ_SLAVE */
    ...
    ret = codec_dai->dai_ops.set_clkdiv(codec_dai, WM8510_MCLKDIV, mclk_div);
    if (ret < 0) {
...
static struct snd_soc_ops playpaq_wm8510_ops = {
    .startup        = playpaq_wm8510_startup,
    .hw_params      = playpaq_wm8510_hw_params,
    .shutdown       = playpaq_wm8510_shutdown,
};
...
static int playpaq_wm8510_init(
